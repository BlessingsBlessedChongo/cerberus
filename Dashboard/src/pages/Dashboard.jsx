...
export default function Dashboard() {
  ...
  // Always read the latest token from localStorage to avoid stale token issues
  const fetchWithAuth = (url) => {
    const currentToken = localStorage.getItem('cerberus_token') || token;
    return fetch(url, {
      headers: {
  ...
  const loadLatest = async () => {
    try {
      ...
      if (data.results && Array.isArray(data.results)) {
        // Reverse array so newer entries overwrite older ones
  ...
  useEffect(() => {
    ...
    // Cards (CPU/RAM/Disk): refresh every 2s so numbers visibly tick in real time
    const cardsInterval = setInterval(loadLatest, 2000);
    // Charts: refresh every 5s so the line graphs update smoothly and often
    const chartsInterval = setInterval(loadHistory, 5000);
    // Alerts: refresh every 5s so new alerts show up almost immediately
    const alertsInterval = setInterval(loadAlerts, 5000);
    // Analytics: heavier computation, keep this less frequent (every 60s)
  ...
  const chartOptions = {
    ...
    // Disable animation so fast-refreshing charts don't jitter/flicker on every update
    animation: false,
    plugins: {
      legend: {
        labels: {
